...
public class ItemFAWEHandler {
    ...
    public static void placeLightsWithFAWE(Player player, List<Location> locations, int lightLevel, int operationId) {
        if (!isFAWEAvailable()) {
            TextHandler.get().sendAndLog(player, "light.error.fawe_not_found");
            return;
        }

        if (locations == null || locations.isEmpty()) {
            TextHandler.get().sendAndLog(player, "light.error.no_locations");
            return;
        }
        ...
        if (bukkitWorld == null) {
            TextHandler.get().sendAndLog(player, "light.error.null_world");
            return;
        }
        ...
        try (EditSession editSession = WorldEdit.getInstance()
            ...
            if (lightType == null) {
                TextHandler.get().sendAndLog(player, "light.error.light_type_not_supported");
                return;
            }
